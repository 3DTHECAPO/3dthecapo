...
function buildDynamicPackage(rawCode){
  ...
  return { tier, title, copy, chips, code: normalized };
}
...
/* FIX:
- mapped codes now work from direct ?code=
- dynamic codes now work from direct ?code=
- scan page flow still works because it already sends ?nfc=1&code=
*/
if(code && codeMap[code]){
  activePackage = { ...codeMap[code], code };
}else if(code){
  ...
  if(saved && saved.tier) activePackage = saved;
}
