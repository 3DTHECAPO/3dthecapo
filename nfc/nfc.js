...
async function postVaultLog(payload, label){
  ...
  if(!res.ok){
    ...
    throw new Error(`${label || 'vault_logs'} failed: ${message || res.status}`);
  }
}

async function logEvent(codeValue, tier, type, record, usedAt){
  const normalizedTier = tier || (record && record.code_type) || '';
  ...
  const fullPayload = Object.assign({}, minimalPayload, {
    ...
    used_at: usedAt || record && record.used_at || null
  });

  try{
    await postVaultLog(fullPayload, 'vault_logs full insert');
  }catch(fullError){
    console.warn('[PLAY3D NFC] Full vault_logs insert failed; retrying minimal payload.', fullError);
    try{
      await postVaultLog(minimalPayload, 'vault_logs minimal insert');
    }catch(minimalError){
      console.warn('[PLAY3D NFC] Minimal vault_logs insert failed.', minimalError);
    }
  }
}


async function patchCodeHit(codeValue, usedAt){
  if(!codeValue) return false;
  const stamp = usedAt || new Date().toISOString();
  try{
    const res = await fetch(`${SUPABASE_URL}/rest/v1/${TABLE}?code=eq.${encodeURIComponent(codeValue)}`,{
      ...
      body:JSON.stringify({ used:true, used_at:stamp })
    });

    if(!res.ok){
      const message = await res.text().catch(()=>res.statusText);
      console.warn('[PLAY3D NFC] vault_codes used_at update failed.', message || res.status);
      return false;
    }
    return true;
  }catch(e){
    console.warn('[PLAY3D NFC] vault_codes used_at update failed.', e);
    return false;
  }
}
...
async function init(){
  ...
  try{
    ...
    if(!record){
      await logEvent(code, '', 'invalid', null);
    ...
    await patchCodeHit(code, usedAt);
    fireBrevoSafe(code, tier);
    await logEvent(code, tier, 'success', record, usedAt);
    unlockUI(tier);
