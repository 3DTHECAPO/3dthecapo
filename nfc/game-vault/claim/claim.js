...
async function postRewardEvent(parsed){
  try{
    const id = identity();
    ...
    Object.keys(payload.reward_metadata).forEach(k=>{
      if(payload.reward_metadata[k] === null || payload.reward_metadata[k] === undefined || payload.reward_metadata[k] === ''){
        delete payload.reward_metadata[k];
      }
    });
    ...
    for(const body of attempts){
      const res = await fetch(`${SUPABASE_URL}/rest/v1/reward_events`,{
        method:'POST',
        headers:{
