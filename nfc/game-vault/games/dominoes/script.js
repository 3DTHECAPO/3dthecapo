...
let mode=(urlMode==='fan'||urlMode==='local')?urlMode:'cpu';
...
function newGame(players,resetMatch){
  const freshUrlMode=(new URLSearchParams(location.search).get('mode')||'').toLowerCase();
  mode=(freshUrlMode==='fan'||freshUrlMode==='local')?freshUrlMode:'cpu';
...
function scheduleCpu(){
  ...
  turnTextEl.textContent='OPPONENT THINKING...';
...
function cpuTurn(){
  ...
  if(move){commitPlay(player,move.tile,move.arm);log(seatName(player)+' played on '+move.arm+'.');render()}
  else{state.passes++;log(seatName(player)+' passed.');render()}
