...
const state={
  ...
  board:{spinnerTile:null,canBranch:false,spinnerArms:{left:[],right:[],top:[],bottom:[]},openEnds:[]}
};
...
function thinkDelay(){return 400+Math.floor(Math.random()*900)}
...
function startWithHighestDouble(){
  ...
  log(seatName(starter.playerIndex)+' opened with spinner '+starter.tile[0]+'-'+starter.tile[1]+'. Count is live after the opener.');
  return true;
}
...
function newGame(players,resetMatch){
  ...
  log(state.players+' player dominoes started. Score only counts open-board ends in multiples of 5. First score must be 10+ to get in.');
  if(state.handNumber===0||state.nextStarterIndex===null) startWithHighestDouble();
  else{
    state.currentPlayerIndex=state.nextStarterIndex;
    log(seatName(state.currentPlayerIndex)+' starts the next hand.');
  }
  state.handNumber++;
  render();
...
function boardEndTotal(){return refreshOpenEnds().reduce((sum,end)=>sum+Number(end.value||0),0)}
function updateBoardTotal(){state.lastBoardTotal=boardEndTotal();return state.lastBoardTotal}
function scoringPointsForTotal(total){return total>0&&total%5===0?total:0}

function showCountPopup(text,good){
  const table=document.querySelector('.casino-table')||document.body;
  const old=document.querySelector('.count-pop');
  if(old)old.remove();
  const pop=document.createElement('div');
  pop.className='count-pop '+(good?'good':'miss');
  pop.textContent=text;
  table.appendChild(pop);
  setTimeout(()=>pop.classList.add('show'),20);
  setTimeout(()=>{pop.classList.remove('show');setTimeout(()=>pop.remove(),300)},1800);
}
...
function awardBoardScore(playerIndex){
  ...
  if(points){
    if(!state.gotIn[playerIndex]&&points<GET_IN_MIN){
      state.lastScoreNote=points+' COUNT — '+seatName(playerIndex)+' still needs 10 to get in.';
      showCountPopup(points+' COUNT — NEED 10 TO GET IN',false);
    ...
    state.lastScoreNote=seatName(playerIndex)+' scored '+points+' from board ends.';
    showCountPopup('+'+points+' COUNT',true);
    log(state.lastScoreNote);
    return points;
  }

  state.lastScoreNote='Ends total '+total+'. No count.';
  if(total>=5) showCountPopup(total+' — NO COUNT',false);
  log(state.lastScoreNote);
  return 0;
}
...
function splitByMatch(tile,match){return tile[0]===match?{match:tile[0],outside:tile[1]}:{match:tile[1],outside:tile[0]}}
function outerValue(tile,arm){return arm==='top'||arm==='left'?tile[0]:tile[1]}
...
function placeOnArm(tile,arm){
  if(arm==='open'&&!state.board.spinnerTile){
    ...
    refreshOpenEnds(); updateBoardTotal(); return true;
  }
...
function endHand(label,winner){
  if(winner!==null&&winner!==undefined){
    ...
    log(seatName(winner)+' won the hand. No random pip-total bonus added.');
  }
  state.handOver=true;
...
function cpuTurn(){
  ...
  if(move){
    ...
    state.passes++; log(seatName(player)+' passed.');
  }
...
function drawTile(){
  ...
  if(!state.stock.length){log('Boneyard empty. Pass if you cannot play.');showCountPopup('BONEYARD EMPTY',false);return}
  const tile=state.stock.pop();
  state.hands[player].push(tile);
  log(seatName(player)+' drew '+tile[0]+'-'+tile[1]+'.');
  render();
  if(legal(tile)){log('Drawn domino is playable.');showCountPopup('DRAWN TILE PLAYS',true)}
  else showCountPopup('DRAWN — NO PLAY',false);
}
function passTurn(){
  const player=state.currentPlayerIndex;
  if(player!==0&&!activeLocal())return;
  if(canPlay(player)){log('You have a playable domino.');return}
  if(state.stock.length){log('Draw before passing.');return}
  state.passes++;
  if(state.passes>=state.players){
    const winner=blockedWinner();
    endHand(winner===null?'BLOCKED HAND - TIE':'BLOCKED HAND - '+seatName(winner)+' WINS',winner);
...
function renderBoard(){
  ...
  if(!state.board.spinnerTile){chainEl.innerHTML='<div class="empty-board">Start a hand.</div>';return}
  chainEl.innerHTML=
...
newBtnEl.onclick=()=>newGame(state.players,false);
