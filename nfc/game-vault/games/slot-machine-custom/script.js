...
const symbolDefs = [
  { name:'speaker', file:'assets/speaker.png', weight:16, payout:3 },
  { name:'crown', file:'assets/crown.png', weight:9, payout:10 },
  { name:'key', file:'assets/key.png', weight:10, payout:8 },
  { name:'lock', file:'assets/lock.png', weight:10, payout:8 },
  { name:'vault', file:'assets/vault.png', weight:7, payout:14 },
  { name:'chain', file:'assets/chain.png', weight:12, payout:5 },
  { name:'mic', file:'assets/mic.png', weight:11, payout:6 },
  { name:'hoodie', file:'assets/hoodie.png', weight:10, payout:7 },
  { name:'cash', file:'assets/cash.png', weight:13, payout:4 },
  { name:'fuck-a-grammy', file:'assets/cover-fuck-a-grammy.png', weight:4, payout:18 },
  { name:'100x3', file:'assets/cover-100x3.png', weight:4, payout:20 },
  { name:'my-resume', file:'assets/cover-my-resume.png', weight:4, payout:20 },
  { name:'logo', file:'assets/logo.png', weight:2, payout:50 }
];
...
function evaluate(board){
  ...
  let totalWin = 0;
  for(const line of paylines(board)) totalWin += lineWin(line, bet);
  const logos = board.flat().filter(s=>s.name==='logo').length;
  if(logos >= 3) totalWin += Math.round(state.jackpot * 0.02);
  return totalWin;
}
function spinOnce(){
...
function autoSpin(count=10){
  ...
  const run = () => {
    ...
    const wait = setInterval(()=>{
      if(!isSpinning){ clearInterval(wait); run(); }
    },120);
  };
  run();
}
