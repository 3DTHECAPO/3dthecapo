(()=>{
  ...
  function placeBid(seat, bid){
    ...
    if(seats.every(s => state.bids[s] !== null)){
      ...
      if(mode === 'cpu' && state.turn !== 'south') setTimeout(botTurn, 450);
    }
    render();
  }
  ...
  function play(seat, card){
    ...
    render();
    if(mode === 'cpu' && state.turn !== 'south') window.setTimeout(botTurn, 500);
  }
  ...
  function finishTrick(){
    ...
    render();
    if(mode === 'cpu' && state.turn !== 'south') window.setTimeout(botTurn, 650);
  }
  ...
  function finishHand(){
    ...
    scoreTeam('EW');
    state.phase = 'over';
    if(window.Play3DPoints && state.score.NS >= state.score.EW) window.Play3DPoints.award('spades', 400, 'contract_hand');
