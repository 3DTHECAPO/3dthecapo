(()=>{
  ...
  function changeBet(delta){
    ...
    const next = Math.max(MIN_BET, Math.min(credits, betAmount + delta));
    betAmount = Math.floor(next / BET_STEP) * BET_STEP || MIN_BET;
    if(betAmount > credits) betAmount = credits;
    setResult('Bet ' + betAmount);
    render(true);
  }
