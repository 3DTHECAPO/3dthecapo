...
(function(){
  ...
  if (unlock === 'track') alert('Exclusive track unlocked');
  if (unlock === 'ep') alert('Early EP preview unlocked');
  if (unlock === 'video') alert('Secret video unlocked');
  if (unlock === 'merch') alert('Secret merch unlocked');
  if (unlock === 'bundle') alert('Bundle unlock active');
})();

// Exact mockup pricing map
(function(){
  const PRODUCTS = [
    { id:'m01', slot:1,  type:'bundle',  name:'Capo Bundle Pack',     price:60, tag:'IN STOCK',    img:'./merch01.png', unlock:'bundle' },
    { id:'m02', slot:2,  type:'hat',     name:'Capo Crown Hat',       price:25, tag:'IN STOCK',    img:'./merch02.png', unlock:'merch' },
    { id:'m03', slot:3,  type:'shirt',   name:'Capo Hustle Tee',      price:20, tag:'IN STOCK',    img:'./merch03.png', unlock:'track' },
    { id:'m04', slot:4,  type:'hoodie',  name:'Capo Hustle Hoodie',   price:40, tag:'IN STOCK',    img:'./merch04.png', unlock:'video' },
    { id:'m05', slot:5,  type:'hat',     name:'Diamond Crown Hat',    price:25, tag:'IN STOCK',    img:'./merch05.png', unlock:'merch' },
    { id:'m06', slot:6,  type:'shirt',   name:'Diamond Money Tee',    price:20, tag:'IN STOCK',    img:'./merch06.png', unlock:'track' },
    { id:'m07', slot:7,  type:'hoodie',  name:'Diamond Money Hoodie', price:40, tag:'IN STOCK',    img:'./merch07.png', unlock:'video' },
    { id:'m08', slot:8,  type:'shirt',   name:'Diamond Shield Tee',   price:20, tag:'IN STOCK',    img:'./merch08.png', unlock:'track' },
      ...
      <article class="card product" data-tag="${p.tag}" data-name="${p.name}" data-type="${p.type}">
        ...
        <div class="body">
          ...
          <div class="actions">
            ...
            <a class="pill small" href="?unlock=${encodeURIComponent(p.unlock || 'merch')}">Unlock</a>
          </div>
        </div>
      </article>`;
    }).join('');
  ...
  document.addEventListener('click', (e) => {
    ...
    if (t?.classList?.contains('quick-view')){
      ...
      if (modalUnlock) modalUnlock.href = `?unlock=${encodeURIComponent(p.unlock || 'merch')}`;
