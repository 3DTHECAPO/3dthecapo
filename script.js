...
const dropCard = document.querySelector('.card.drop');
if (dropCard){
...
(function(){
  ...
  if (grid){
    const order = {ep:0, bundle:1, shirt:2, hat:3, hoodie:4, comingsoon:5};

    PRODUCTS.sort((a,b)=>{
      const liveA = ['EP','IN STOCK','LOW STOCK','SOLD OUT'].includes(a.tag);
      const liveB = ['EP','IN STOCK','LOW STOCK','SOLD OUT'].includes(b.tag);
